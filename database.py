...
class DatabaseManager:
    ...
    def add_customer(self, national_id, name):
        """Add a new customer"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO customers (national_id, name)
                VALUES (?, ?)
            ''', (national_id, name))
            conn.commit()
    ...
    def get_customer(self, national_id):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT national_id, name, created_at, updated_at
                FROM customers
                WHERE national_id = ?
            ''', (national_id,))
    ...
    def update_customer(self, national_id, name):
